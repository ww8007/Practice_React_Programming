...
export default function TodoList() {
   ...
   const onClick = () => {
      ...
      import("./Todo.js").then(({ Todo }) => {
         ...
         const newTodo = (
            <Todo key={position} title={`할 일 ${position}`}></Todo>
         ); //- (4)
...
-  NODE_ENV 환경 변수 외에 다른 환경 변수는 REACT_APP\* 접두사를 꼭 붙여야 한다.
-  코드에서는 `js proces.env.REACT_APP_ `형태로 접근이 가능
-  환경변수는 셸에서 입력하거나 .env 파일을 이용해 접근이 가능
-  환경변수가 많아지면 .env 파일을 이용하것이 좋음

### 단일 페이지 애플리케이션 만들기

-  리엑트 애플리케이션의 페이지 전환은 단일 페이지 애플리케이션(single page application, SPA)방식으로 개발하는 것이 정석
-  단일 페이지 app은 초기 요청 시 서버에서 첫 페이지 처리
-  이후의 라우팅은 클라이언트에서 처리하는 웹 에플리케이션
-  전통적인 방식은 페이지 전환 -> 렌더링 결과 서버에서 받아옴
...
-  단일 페이지 에ㅐ플리케이션이 구현이 가능하려면 두 가지 기능이 필요

   1. 자바스크립트에서 브라우저로 페이지 전환 요청을 보낼 수 있다.
      -  단 브라우저는 서버로 요청을 보내지 않아야 한다.
   2. 브라우저의 뒤로 가기와 같은 사용자 페이지 전환 요청을 자바스크립트에서 처리 할 수 있다.
      -  브라우저는 서버로 요청을 하지 않아도 된다.

-  위의 조건을 만족하는 브라우저 API는 pushState, replaceState, pop state 이벤트
-  브라우저에는 state를 저장하는 stack(스택)이 존재

### react-router-dom

-  브라우저 히스토리 API를 이용해서 페이지 라우팅 처리를 직접 구현할 수도 있지만
-  신경쓸 부분이 UP

-  이럴 때 사용하는 것이 reac-router-dom
-  리액트로 단일 페이지 APP을 만들 때 많이 사용
...
function Photo({ match }) {
   return <h2>사진을 감상하세요.</h2>;
}
```

-  [1] : react-router-dom을 사용하기 위해서는 전체를 BrowserRouter 컴포넌트로 감싸야함
-  [2] : 버튼을 통해서 페이지를 전환 할 때는 Link 컴포넌트를 사용해야 함
-  [3] : Route 컴포넌트를 이용해 각 페이지 정의
   -  현재 주소가 path 속성값으로 시작하면 component 속성값이 가리키는 컴포넌트를 랜더링
   -  (/) 슬래쉬 단위로 비교를 하기 때문에 완전히 일치해야 컴포넌트 랜더링
   -  exact 속성값을 입력하지 않으면 Home 컴포넌트는 항상 렌더링

> 같은 path 속성값을 가지는 Route 컴포넌트를 여러번 작성 가능

    동일 경로로 들어올 경우 두개의 컴포넌트가 랜더링 됨
